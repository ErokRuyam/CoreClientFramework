//
//  CoreClientTests.swift
//  CoreClientTests
//
//  Created by Mayur on 03/10/17.
//  Copyright © 2017 Odocon. All rights reserved.
//

import XCTest
@testable import CoreClient

class CoreClientTests: XCTestCase {
    
    override func setUp() {
        super.setUp()
        // Put setup code here. This method is called before the invocation of each test method in the class.
    }
    
    override func tearDown() {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
        super.tearDown()
    }
    
    func testExample() {
        // This is an example of a functional test case.
        // Use XCTAssert and related functions to verify your tests produce the correct results.
        let clientConfig = ClientConfiguration.init()
        clientConfig.persistentStore = SQLiteManager.init()
        
        let client = TestClient.init(aClientConfiguration: clientConfig, theCallbackQueue: nil)

        let getPlaceTask = client.getLocationInfo()
        getPlaceTask?.executeWithCompletionHandler({ (result, error) in
            print("Result: \(String(describing: result)) \n Error: \(String(describing: error))")
        })
        
        sleep(15)
    }
    
    func testPerformanceExample() {
        // This is an example of a performance test case.
        self.measure {
            // Put the code you want to measure the time of here.
        }
    }
}
